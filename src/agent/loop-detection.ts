...
export function hashTollCall(toolName: string, params: unknown): string {
...
export function recordCall(toolName: string, params: unknown): void {
  history.push({
    toolName,
    argsHash: hashTollCall(toolName, params),
    timestamp: Date.now(),
  });
...
export function recordResult(
  ...
  const argsHash = hashTollCall(toolName, params);
...
function getNoProgressStreak(toolName: string, argsHash: string): number {
  ...
  for (let i = history.length - 1; i >= 0; i--) {
    const r = history[i];
    if (toolName !== toolName || r.argsHash !== argsHash) continue;
...
export function detect(toolName: string, params: unknown): DetectionResult {
  ...
  if (pingPong >= CRITICAL_THRESHOLD) {
    return {
      ...
      detector: "ping_pong",
      count: pingPong,
      message: `[熔断] 检测到兵乓循环（${pingPong} 次交替），强制停止`,
    };
  }
